...
add_action('wp_ajax_ecommerce_prime_latest_posts', 'ecommerce_prime_latest_posts_callback');
add_action('wp_ajax_nopriv_ecommerce_prime_latest_posts', 'ecommerce_prime_latest_posts_callback');

// Recommendec Post Ajax Call Function.
function ecommerce_prime_latest_posts_callback() {

    $twp_paged = absint( $_POST['page'] );
    $recommended_post_category = esc_html( $_POST['category'] );
    $recommended_post_query = new WP_Query( array( 'post_type' => 'post','posts_per_page' => 3, 'category_name' => esc_html( $recommended_post_category ), 'paged'=> absint( $twp_paged ) ) );
    ...
    if ( $recommended_post_query->have_posts() ) :
        ...
        while ( $recommended_post_query->have_posts() ) : $recommended_post_query->the_post();
            ...
            <div class="column column-4 column-sm-6 column-xs-12 latest-news-load wow fadeInUp ajax-added-posts" data-wow-delay="<?php echo esc_attr( $delay ); ?>" >
...
add_action('wp_ajax_ecommerce_prime_tab_posts', 'ecommerce_prime_tab_posts_callback');
add_action('wp_ajax_nopriv_ecommerce_prime_tab_posts', 'ecommerce_prime_tab_posts_callback');

// Recommendec Post Ajax Call Function.
function ecommerce_prime_tab_posts_callback() {
