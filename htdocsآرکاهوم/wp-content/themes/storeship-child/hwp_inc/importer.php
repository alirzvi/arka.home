...
function hamyarwp_import_files() {
	return array(
		array(
			...
			'import_notice'                => 'لطفا قبل درون ریزی افزونه های مورد نیاز را نصب کنید :از بخش افزونه های مورد نیاز',
			'preview_url'                  => '#',
		),
	);
}
...
function hamyarwp_after_import_setup() {
	...
	$secondary_menu = get_term_by( 'name', 'Social Menu', 'nav_menu' );
	$top_header_menu = get_term_by( 'name', 'منوی فوتر', 'nav_menu' );

	set_theme_mod( 'nav_menu_locations', array(
			'Primary Menu' => $primary_menu->term_id,
			'Social Menu' => $secondary_menu->term_id,
			'منوی فوتر' => $top_header_menu->term_id,
		)
	);
