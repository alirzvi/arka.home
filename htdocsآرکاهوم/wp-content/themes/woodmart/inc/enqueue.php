...
if ( ! function_exists( 'woodmart_enqueue_base_styles' ) ) {
	function woodmart_enqueue_base_styles() {
		...
		// Frontend admin bar.
		if ( is_admin_bar_showing() ) {
			wp_enqueue_style( 'woodmart-frontend-admin-bar', WOODMART_ASSETS . '/css/style-frontend.css', array(), $version );
		}
...
if ( ! function_exists( 'woodmart_force_enqueue_styles' ) ) {
	...
	function woodmart_force_enqueue_styles() {
		...
		if ( is_active_widget( 0, 0, 'woodmart-woocommerce-layered-nav' ) || is_active_widget( 0, 0, 'woocommerce_product_categories' ) ) {
			woodmart_force_enqueue_style( 'widget-wd-layered-nav-product-cat' );
		}
		...
		if ( woodmart_get_opt( 'collapse_footer_widgets' ) ) {
			woodmart_force_enqueue_style( 'footer-widget-collapse' );
		}
		...
		if ( function_exists( '_mc4wp_load_plugin' ) && ! get_option( 'wd_import_theme_version' ) ) {
			woodmart_force_enqueue_style( 'mc4wp-deprecated', true );
		...
		if ( woodmart_woocommerce_installed() ) {
			...
			$woodmart_demos="";
			 $version = woodmart_get_theme_info( 'Version' );
						$woodmart_demos= woodmart_get_opt( 'demo_woodmart_select' );
	if($woodmart_demos=="inherit")
	{
	   
	    wp_enqueue_style( 'digikala_demo', WOODMART_THEME_DIR . '/css/digikala.css', array(), $version );
		
	}elseif($woodmart_demos=="default")
	{
	    wp_enqueue_style( 'mootan_demo', WOODMART_THEME_DIR . '/css/mootan.css', array(), $version );
		
	}
	elseif($woodmart_demos=="default-alt")
	{
	    wp_enqueue_style( 'custom_demos', WOODMART_THEME_DIR . '/css/custom.css', array(), $version );
		

	}
		elseif($woodmart_demos=="base")
	{wp_enqueue_style( 'base', WOODMART_THEME_DIR . '/css/base.css', array(), $version );
	...
	{
	    wp_enqueue_style( 'small-image', WOODMART_THEME_DIR . '/css/main-demo.css', array(), $version );
